...
function resolve(inliner, todo, $) {
  ...
  return todo.map(function videos(video) {
    ...
    if (inliner.options.skipAbsoluteUrls &&
      ...
      promises.push(inliner.image(url, inliner.options).then(function then(dataURL) {
    ...
    if (inliner.options.skipAbsoluteUrls &&
      ...
      promises.push(inliner.image(posterUrl, inliner.options).then(function then(dataURL) {
