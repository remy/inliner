...
function image(url, options) {
  ...
  return this.get(url, { encoding: 'binary' }).then(function then(res) {
    ...
    if (options['resize-image-width']) {
      // otherwise we base64 encode the imagie
      return new Promise(function (resolve, reject) {
        if (!(res.body)) {
          resolve(url)
        } else {
          console.log()
          sharp(res.body).resize(options['resize-image-width'])
            .jpeg({quality: options['resize-quality'] || 80}).toBuffer()
            .then(function (b) {
              resolve('data:image/jpeg;base64,' + b.toString('base64'));
            }).catch(function (e) {resolve(url)})
        }
      });
    }
